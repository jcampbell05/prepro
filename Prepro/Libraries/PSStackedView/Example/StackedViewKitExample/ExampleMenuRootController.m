...
- (void)viewDidLoad {
    ...
    self.stackController.enableDraggingPastInsets = NO;
    self.stackController.enableBounces = NO;
    self.stackController.enableScalingFadeInOut = NO;
    self.stackController.enableShadows = NO;
    self.stackController.enablePopOverlapedViewOnTap = NO;
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {  
    ...
    if (viewController) {
        ...
        if ( XAppDelegate.stackController.viewControllers.count > 0) {
            [XAppDelegate.stackController popToRootViewControllerAnimated:NO];
            [XAppDelegate.stackController pushViewController:viewController fromViewController:nil animated:NO];
        } else {
